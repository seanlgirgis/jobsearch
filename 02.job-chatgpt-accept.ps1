...
$defaultIntake = "intake\intake.md"

function Resolve-JobSourceFromIntake {
    ...
    # Strip UTF-8 BOM if present and normalize signal text.
    $text = $text.TrimStart([char]0xFEFF)
    $lines = $text -split "`r?`n" | ForEach-Object { $_.Trim().TrimStart([char]0xFEFF) } | Where-Object { $_ }
    $firstLine = ""
    if ($lines) { $firstLine = $lines[0] }
...
function Test-IsSafeIntakePath {
    ...
    try {
        ...
        $intakeRoot = (Resolve-Path -LiteralPath (Join-Path $projectRoot "intake")).Path
        return $resolved.StartsWith($intakeRoot, [System.StringComparison]::OrdinalIgnoreCase)
    } catch {
        return $false
    }
}

if (-not $PSBoundParameters.ContainsKey("IntakeFile") -or [string]::IsNullOrWhiteSpace($IntakeFile)) {
    # Strong default: always prefer canonical intake/intake.md when present.
    if (Test-Path -LiteralPath $defaultIntake) {
        $IntakeFile = $defaultIntake
    }

    # Optional fallback to cache intake ONLY when path is inside intake/.
...
if (-not (Test-IsSafeIntakePath -CandidatePath $IntakeFile)) {
    Write-Host "ERROR: Refusing unsafe intake path outside intake\\ : $IntakeFile" -ForegroundColor Red
    Write-Host "Use -IntakeFile intake\\intake.md (or another file under intake\\)." -ForegroundColor Yellow
    exit 1
}
...
Write-Host ""
Write-Host "Next: .\03.job-chatgpt-render.ps1 -NoStrict " -ForegroundColor Yellow
