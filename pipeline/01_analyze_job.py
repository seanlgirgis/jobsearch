...
def _build_messages(
    ...
    return [
        ...
        },
        {
            "role": "user",
            "content": (
                f"Analyze the job and candidate context in one pass and return exactly one JSON object.\n"
                f"Required output shape example:\n{json.dumps(schema_hint, indent=2)}\n\n"
                f"Constraints:\n"
                f"- Set artifact_type='job_packet' and artifact_schema_version='v1'.\n"
                f"- Set job_id='{job_id}' and job_hash='{job_hash}'.\n"
                f"- score is integer 0-100.\n"
                f"- llm_decision must be one of: proceed, hold, skip.\n"
                f"- rationale must be short and grounded.\n\n"
